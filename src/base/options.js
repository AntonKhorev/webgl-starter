...
class Options {
	constructor() {
		...
		const simpleMakeEntry=description=>{
			...
			const ctorArgs=[null,()=>true,simpleUpdateCallback,name,contents,defaultValue];
					...
					if (nArrays==0) {
						if (className=='Array') {
							...
							arg.forEach(x=>{
								const type=x[1];
								contents.push(type);
								defaultValueOrConstructors[type]=()=>simpleMakeEntry(x);
							});
						} else {
							contents=arg.map(x=>{
								if (Array.isArray(x)) {
